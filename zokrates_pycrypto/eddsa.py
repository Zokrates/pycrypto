...
class PrivateKey(namedtuple("_PrivateKey", ("fe"))):
    ...
    def sign(self, msg, B=None):
        ...
        B = B or BabyJubJub.generator()

        A = PublicKey.from_private(self)  # A = kB

        M = msg
        r = hash_to_scalar(self.fe, M) % BabyJubJub.JUBJUB_L.n  # r = H(k,M) mod L
        R = B.mult(r)  # R = rB
...
def hash_to_scalar(*args):
    ...
    return int(digest.hex(), 16) % BabyJubJub.JUBJUB_E # mod JUBJUB_E here for optimized implementation
